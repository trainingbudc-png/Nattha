window.onload = function() {
    ...
    verifyRoleSilently();
    setTimeout(() => { loadUserTableData(); }, 100);
};
...
async function loadUserTableData() {
    ...
    activeCardsContainer.innerHTML = '<div class="col-12 text-center py-5 text-muted bg-white rounded-4 border shadow-sm">กำลังเชื่อมต่อข้อมูล... ⏳</div>';
    historyTbody.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-muted" style="justify-content: center !important;">กำลังเชื่อมต่อข้อมูล... ⏳</td></tr>';
    ...
    try {
        ...
        if (res.status === "success" && res.data && res.data.length > 0) {
            ...
            if (myData.length === 0) {
                activeCardsContainer.innerHTML = '<div class="col-12 text-center py-5 text-muted bg-white rounded-4 border shadow-sm">🎉 ไม่มีรายการที่กำลังดำเนินการ</div>';
            ...
            let activeList = []; let historyList = [];
            myData.forEach(item => {
        ...
        activeCardsContainer.innerHTML = '<div class="col-12 text-center text-danger py-5 bg-white rounded-4 border shadow-sm">❌ เกิดข้อผิดพลาดในการเชื่อมต่อข้อมูล</div>';
...
function renderActiveUserCards(data) {
    ...
    if (data.length === 0) { container.innerHTML = '<div class="col-12 text-center text-muted py-5 bg-white rounded-4 border shadow-sm">🎉 ไม่มีรายการที่กำลังดำเนินการ</div>'; return; }
    let htmlContent = "";
    data.forEach(item => {
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="active-task-card bg-white rounded-4 shadow-sm position-relative overflow-hidden" style="border: 1px solid #e2e8f0;">
                    ...
                    <div class="p-3 ps-4 d-flex justify-content-between align-items-center" style="cursor: pointer; user-select: none;" data-bs-toggle="collapse" data-bs-target="#collapse-${safeId}" onclick="closeOtherAccordions('collapse-${safeId}')">
                        <div><div class="fw-bold text-dark d-flex align-items-center gap-2" style="font-size: 1.15rem;">${item.reqId} <span class="collapse-icon text-muted" style="font-size: 0.75rem;">▼</span></div><div class="text-secondary mt-1" style="font-size: 0.85rem; font-weight: 500;">สถานะ: <span class="text-danger">${stepLabel}</span></div></div>
                    </div>
                    <div class="px-3 pb-3 ps-4"><div onclick="event.stopPropagation();">${actionBtn}</div></div>
                    <div id="collapse-${safeId}" class="collapse"><div class="p-3 pt-3 ps-4 border-top border-light bg-light bg-opacity-50"><div class="d-flex justify-content-between align-items-center mb-1"><span class="text-muted" style="font-size: 0.85rem;">📅 เวลา:</span><span class="text-dark" style="font-size: 0.85rem; font-weight: 500;">${dateStr}</span></div>${formattedIpads}</div></div>
                </div>
            </div>`;
    });
...
function closeOtherAccordions(targetId) { document.querySelectorAll('#activeCardsContainer .collapse.show').forEach(el => { if (el.id !== targetId) { let bsCollapse = bootstrap.Collapse.getInstance(el) || new bootstrap.Collapse(el, {toggle: false}); bsCollapse.hide(); } }); }

function renderUserTableRows(dataList, tbodyElement) {
    if (dataList.length === 0) { tbodyElement.innerHTML = `<tr><td colspan="4" class="text-center text-muted py-4" style="justify-content: center !important;">ยังไม่มีประวัติรายการเสร็จสิ้น</td></tr>`; return; }
    let htmlContent = "";
    dataList.forEach(item => {
        ...
        if (statusTxt.includes("เคลียร์") || statusTxt.includes("คืนแล้ว") || statusTxt.includes("เสร็จสิ้น")) { displayStatus = "คืนเรียบร้อย"; badgeClass = "bg-light text-secondary border"; } else if (statusTxt.includes("ยกเลิก")) { displayStatus = "ยกเลิกรายการ"; badgeClass = "bg-light text-secondary border"; }
        htmlContent += `<tr class="text-center align-middle"><td data-label="📌 เลขรายการ" class="fw-bold text-dark">${item.reqId}</td><td data-label="📦 อุปกรณ์ที่ยืม">${typeHtml}</td><td data-label="📱 เครื่องที่เตรียม">${idsHtml}</td><td data-label="📊 สถานะ"><span class="badge ${badgeClass} px-3 py-2 rounded-pill shadow-sm">${displayStatus}</span></td></tr>`;
    });
